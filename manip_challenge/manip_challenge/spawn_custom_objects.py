...
def parse_args():
    ...
    p.add_argument("--model-list", nargs="+", default=None, help="List of models and counts, e.g. banana:1 coke_can:2")
    p.add_argument("--num-spawns", type=int, default=None, help="Total number to spawn (default: sum of model-list)")
    ...
    parsed = p.parse_args(non_ros_argv[1:])
    
    # Process model_list into a dictionary
    if parsed.model_list is None:
        parsed.model_list = MODEL_LIST
    elif isinstance(parsed.model_list, list):
        parsed_dict = {}
        for item in parsed.model_list:
            if ':' in item:
                name, count = item.split(':')
                parsed_dict[name] = int(count)
            else:
                parsed_dict[item] = 1
        parsed.model_list = parsed_dict

    # Compute num_spawns from model_list if not explicitly set
    if parsed.num_spawns is None:
        parsed.num_spawns = sum(parsed.model_list.values())
