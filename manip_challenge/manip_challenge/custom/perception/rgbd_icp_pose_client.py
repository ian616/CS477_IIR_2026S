#!/usr/bin/env python3
"""Client for the YOLO RGB-D ICP pose JSON service."""
...
class RgbdIcpPoseClient(Node):
    def __init__(self, service_name):
        super().__init__("rgbd_icp_pose_client")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Request a YOLO RGB-D ICP pose estimate.")
    parser.add_argument("command", nargs="?", default="banana", help="Target or command, e.g. coke_can or 'pick banana'.")
    parser.add_argument("--service-name", default="detect_object_icp_pose")
    args = parser.parse_args(cli_args)

    rclpy.init()
    node = RgbdIcpPoseClient(args.service_name)
    try:
