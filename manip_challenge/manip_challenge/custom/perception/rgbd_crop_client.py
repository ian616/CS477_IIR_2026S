#!/usr/bin/env python3
"""Client for the YOLO RGB-D crop JSON service."""
...
class RgbdCropClient(Node):
    def __init__(self, service_name):
        super().__init__("rgbd_crop_client")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Request a YOLO RGB-D crop and save it on the server.")
    parser.add_argument("command", nargs="?", default="banana", help="Target or command, e.g. coke_can or 'pick banana'.")
    parser.add_argument("--service-name", default="detect_object_rgbd_crop")
    args = parser.parse_args(cli_args)

    rclpy.init()
    node = RgbdCropClient(args.service_name)
    try:
